...
//*********************************************************************************************************************************
// o SHA256_K	: 단계연산에서 사용되는 64개의 상수
//*********************************************************************************************************************************
const unsigned int SHA256_K[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
...
{
	unsigned int a, b, c, d, e, f, g, h, T1, X[64];
	unsigned int j;
...
// o 입력				: AlgInfo		-  SHA-256 구조체의 포인터 변수
// o 출력				: 
//*********************************************************************************************************************************
...
//*********************************************************************************************************************************
// o SHA256_Update()	: 임의의 길이를 가지는 입력 메시지를 512 비트 블록 단위로 나누어 압축함수를 호출하는 함수
// o 입력				: AlgInfo		- SHA-256 구조체의 포인터 변수
...
//*********************************************************************************************************************************
// o SHA256_Final()		: 메시지 덧붙이기와 길이 덧붙이기를 수행한 후 마지막 메시지 블록을 가지고 압축함수를 호출하는 함수
// o 입력				: AlgInfo	- SHA-256 구조체의 포인터 변수
//						  Digest	- SHA-256 해쉬값을 저장할 포인터 변수
// o 출력				:
//*********************************************************************************************************************************
void SHA256_Final(SHA256_ALG_INFO *AlgInfo, unsigned char* Digest)
{
	unsigned int i, Index, CountL, CountH;
	...
	// 메시지 덧붙이기
	Index = (CountL >> 3) % SHA256_DIGEST_BLOCKLEN;
	AlgInfo->Buffer[Index++] = 0x80;

	// 덧붙이기로 인해 메시지 블록이 512 비트가 넘으면 압축 함수를 호출
	if (Index > SHA256_DIGEST_BLOCKLEN - 8)
	{
		memset((unsigned char*)AlgInfo->Buffer + Index, 0, (int)(SHA256_DIGEST_BLOCKLEN - Index));
		SHA256_Transform((unsigned int*)AlgInfo->Buffer, AlgInfo->ChainVar);
		memset((unsigned char*)AlgInfo->Buffer, 0, (int)SHA256_DIGEST_BLOCKLEN - 8);
	}
	else
		memset((unsigned char*)AlgInfo->Buffer + Index, 0, (int)(SHA256_DIGEST_BLOCKLEN - Index - 8));
	...
	// 메시지 길이 덧붙이기
	((unsigned int*)AlgInfo->Buffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = CountH;
	((unsigned int*)AlgInfo->Buffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = CountL;
